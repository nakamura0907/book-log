...
class BooksUseCase {
    ...
    async addBook(req: AddBookRequest) {
        // バリデーション
        const coverImageFile = req.file ? CoverImageFile.validate(req.file, this.storage.COVER_IMAGE_URL) : undefined;
    ...
    async fetchBookList(options: FetchOptionsRequest) {
        // バリデーション
        const fetchOptions = new FetchOptions(options.skip, options.query, options.status, options.order);

        // データベース取得
        const books = await this.repository.fetchBookList(fetchOptions);

        return { books : books.map((book) => bookResponse(book)) };
    }
