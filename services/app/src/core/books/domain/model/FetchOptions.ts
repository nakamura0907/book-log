...
class FetchOptions {
    ...
    constructor(skip?: number, query?: string, status?: number, order?: string) {
        if (skip && !this.validateNum(skip)) throw new Exception("skipは0以上の整数で入力してください", 400);
        if (status && !this.validateNum(status)) throw new Exception("statusは0以上の整数で入力してください", 400);
        if (order && !this.validateOrder(order)) throw new Exception("orderはcreatedAt, -createdAt, score, -score, -updatedAtのいずれかで入力してください", 400);

        this._skip = skip;
        this._query = query;
        this._status = status;
        this._order = order ? (order as Order) : "-createdAt";
    }
    ...
    private validateOrder(value: string): value is Order {
        return ["createdAt", "-createdAt", "score", "-score", "-updatedAt"].includes(value);
    }
}
